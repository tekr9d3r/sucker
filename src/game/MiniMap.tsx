...
export const MiniMap = () => {
  ...
  const catX = useGameStore((s) => s.catX);
  const catZ = useGameStore((s) => s.catZ);
  ...
  useEffect(() => {
    ...
    if (dirt) {
      ...
      for (let i = 0; i < dirt.cells.length; i++) {
        ...
        ctx.fillStyle = cleaned ? "rgba(120,200,140,0.55)" : "rgba(80,55,30,0.5)";
    ...
    ctx.fill();
    ctx.restore();
  }, [playerX, playerZ, playerAngle, catX, catZ, cellsVersion]);
